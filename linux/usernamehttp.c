...
int main(int argc, char **argv) {
	...
	char buff[MAXLINE], recvline[MAXLINE], cmd[16], path[64], vers[16], hostHeading[16], hostResult[20], usernameHeading[20], usernameResult[20], passwordHeading[20], passwordResult[20];
	char path1[64] = ".";
	FILE * hFile;
	int char_in, count = 0;
	...
	for ( ; ; ) {
		...
		sscanf(recvline,"%s %s %s %s %s %s %s %s %s", cmd, path, vers, hostHeading, hostResult, usernameHeading, usernameResult, passwordHeading, passwordResult); //parse command 

		printf("\n%s %s\n", hostHeading, hostResult);
		printf("\n%s %s\n", usernameHeading, usernameResult);
		printf("\n%s %s\n", passwordHeading, passwordResult);
